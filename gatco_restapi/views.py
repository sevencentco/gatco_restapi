...
class API(ModelView):
    ...
    def __init__(self, session, model, exclude_columns=None,
        ...
        # move post_form_preprocessor to preprocess['POST'] for backward
        # compatibility
        if post_form_preprocessor:
            msg = ('post_form_preprocessor is deprecated and will be removed'
                   ' in version 1.0; use preprocess instead.')
            warnings.warn(msg, DeprecationWarning)
            self.preprocess['POST'].append(post_form_preprocessor)
        # postprocess for PUT are applied to PATCH because PUT is just a
        # redirect to PATCH
